...
export const ModuleSettings: React.FC<ModuleSettingsProps> = ({
  ...
  // Active Settings Sub-Tab / Filter
  ...
  // Keep state synced whenever incoming settings update
  useEffect(() => {
    if (settings) {
      ...
      if (settings.tawkSiteId) setTawkSiteId(settings.tawkSiteId);
      if (settings.tawkApiKey) setTawkApiKey(settings.tawkApiKey);
      if (settings.ticketEmail) setTicketEmail(settings.ticketEmail);
      if (settings.noticeMarquee) setNoticeMarquee(settings.noticeMarquee);
      if (typeof settings.maintenanceMode === 'boolean') setMaintenanceMode(settings.maintenanceMode);
      if (settings.themeColor) setActiveTheme(settings.themeColor);
      if (settings.adminUpiId) setAdminUpiId(settings.adminUpiId);
      if (settings.adminUpiName) setAdminUpiName(settings.adminUpiName);
      if (settings.adminQrCodeUrl) setAdminQrCodeUrl(settings.adminQrCodeUrl);
      if (settings.adminBankName) setAdminBankName(settings.adminBankName);
      if (settings.adminAccountNo) setAdminAccountNo(settings.adminAccountNo);
      if (settings.adminIfsc) setAdminIfsc(settings.adminIfsc);
      if (settings.adminAccountHolder) setAdminAccountHolder(settings.adminAccountHolder);
      if (settings.adminUpiNote) setAdminUpiNote(settings.adminUpiNote);
      if (settings.minDeposit) setMinDeposit(settings.minDeposit);
      if (settings.depositMultiple) setDepositMultiple(settings.depositMultiple);
      if (settings.maxDeposit) setMaxDeposit(settings.maxDeposit);
      if (settings.minWithdrawal) setMinWithdrawal(settings.minWithdrawal);
      if (settings.withdrawalMultiple) setWithdrawalMultiple(settings.withdrawalMultiple);
      if (typeof settings.tdsPercentage === 'number') setTdsPercentage(settings.tdsPercentage);
      if (typeof settings.adminFeePercentage === 'number') setAdminFeePercentage(settings.adminFeePercentage);
  ...
  const SETTINGS_SUB_TABS = [
    ...
    { id: 'ticket_theme', label: '🎟️ Ticket Theme (टिकट रंग)', icon: TicketIcon },
    { id: 'branding', label: '🌐 Branding & Contacts (ब्रांडिंग)', icon: Globe },
    { id: 'security', label: '🔐 Admin ID & Password (सुरक्षा)', icon: Shield },
    { id: 'audit', label: '📜 Login Audit Log (ऑडिट)', icon: History },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-1.5 rounded-2xl bg-slate-900/90 border border-slate-800 flex items-center gap-1.5 overflow-x-auto shadow-lg">
      ...
      {(activeSection === 'all' || activeSection === 'games') && (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10px] bg-black/40 p-2 rounded-xl border border-white/5">
                        <div>
                          ...
                          <span className="font-black text-amber-400 font-mono">₹{(g.prizePool || 0).toLocaleString('en-IN')}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block">Registered:</span>
                          <span className="font-bold text-white">{g.registeredPlayers || g.soldTickets || 0} Players</span>
                        </div>
                      </div>
                    </div>
      ...
      <form onSubmit={handleSaveSettings} className="space-y-6">
        ...
        {/* 🎟️ SECTION 3: TICKET COLOR THEMES */}
        {(activeSection === 'all' || activeSection === 'ticket_theme') && (
          <div className="rounded-3xl bg-slate-900/90 border-2 border-amber-500/40 p-5 sm:p-7 space-y-4 shadow-xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-amber-500/20 pb-4">
              <div className="flex items-center gap-3">
                <div className="p-2.5 rounded-2xl bg-amber-500/20 text-amber-400 border border-amber-500/30">
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-2xl border text-left flex flex-col justify-between transition-all cursor-pointer ${
                      isSelected
                        ? 'bg-slate-900 border-2 border-amber-400 shadow-xl shadow-amber-500/20 scale-[1.03]'
                        : 'bg-slate-950 border-slate-800 hover:border-slate-700 hover:bg-slate-900/60'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="w-4 h-4 rounded-full shadow border border-white/20" style={{ backgroundColor: palette.previewHex }} />
                      {isSelected ? (
                    ...
                    <span className="text-[10px] text-slate-400 truncate mt-0.5">{palette.badgeLabel}</span>
                  </button>
                );
              })}
            </div>
        ...
        {/* 🌐 SECTION 4: PLATFORM BRANDING & MARQUEE */}
        {(activeSection === 'all' || activeSection === 'branding') && (
          <div className="rounded-3xl bg-slate-900/90 border border-slate-800 p-5 sm:p-6 space-y-5 shadow-xl">
            ...
            {/* Tawk.to Live Chat & Ticketing Desk Integration */}
      ...
      {(activeSection === 'all' || activeSection === 'security' || activeSection === 'audit') && (
